...
class SetupCallback(Callback):
    ...
    def save_code_snapshot(self):
        ...
        # 需要保存的文件列表（相对路径）
        files_to_snapshot = [
            "train_sft_modify_13ch.sh",
            "main_sft_13ch.py",
            "configs/train_sft_13ch.yaml",
            "ldm/data/sft_dataset_13ch.py",
            "ldm/models/diffusion/ddpm_sft_13ch.py",
        ]
...
if __name__ == "__main__":
    ...
    default_callbacks_cfg = {
        "setup_callback": {
            "target": "main_sft_13ch.SetupCallback",
            "params": {
                "resume": opt.resume,
                "now": now,
                "logdir": logdir,
        ...
        "image_logger": {
            "target": "main_sft_13ch.ImageLogger",
            "params": {
                "batch_frequency":50,
                "max_images": 3,
                "clamp": True,
        ...
        "cuda_callback": {
            "target": "main_sft_13ch.CUDACallback"
        },
    }
    if version.parse(pl.__version__) >= version.parse('1.4.0'):
