...
def compute_features_wrapp(path,mask_path, IDLoss_model,Other_model, batch_size, dims, device,
                               num_workers=1,data_name="celeba", max_samples=None):
...
def calculate_id_given_paths(paths, batch_size, device, dims, num_workers=1,data_name="celeba",args=None, max_samples=None):
    ...
    feat1,ori_lab = compute_features_wrapp(paths[0],paths[2], IDLoss_model,None, batch_size,
                                        dims, device, num_workers,data_name=data_name, max_samples=max_samples)
