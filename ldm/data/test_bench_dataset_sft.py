...
class FFHQdataset(data.Dataset):
    def __init__(self,state,load_vis_img=False,label_transform=None,fraction=1.0,**args
        ...
        # 可以通过args传入子文件夹名称，默认使用images512
        img_subfolder = args.get('img_subfolder', 'images512')
        mask_subfolder = args.get('mask_subfolder', 'BiSeNet_mask')
        
        if state == "train":
            self.imgs = sorted([osp.join(args['dataset_dir'], img_subfolder, '{0:0=5d}.png'.format(idx)) for idx in range(68000)])
            self.labels = sorted([osp.join(args['dataset_dir'], mask_subfolder,'{0:0=5d}.png'.format(idx))  for idx in range(68000)])

        elif state == "validation":
            self.imgs = sorted([osp.join(args['dataset_dir'], img_subfolder, '{0:0=5d}.png'.format(idx)) for idx in range(68000,70000)])
            self.labels = sorted([osp.join(args['dataset_dir'], mask_subfolder,'{0:0=5d}.png'.format(idx))  for idx in range(68000,70000)])
            
        else:
            
            self.imgs = sorted([osp.join(args['dataset_dir'], img_subfolder,'{0:0=5d}.png'.format(idx))  for idx in range(68000,69000)])
            self.labels = sorted([osp.join(args['dataset_dir'], mask_subfolder, '{0:0=5d}.png'.format(idx))  for idx in range(68000,69000)]) 
            

            self.ref_imgs = sorted([osp.join(args['dataset_dir'], img_subfolder, '{0:0=5d}.png'.format(idx))  for idx in range(69000,70000)])
            self.ref_labels =  sorted([osp.join(args['dataset_dir'], mask_subfolder, '{0:0=5d}.png'.format(idx))  for idx in range(69000,70000)]) 
